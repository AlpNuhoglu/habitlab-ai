...
- **`runAsTenant(userId, fn)`** (`infrastructure/database/tenant-context.ts`) for workers acting on one user's behalf (notification scheduler per habit, `AssignmentService` writes, and both event-stream workers). Preferred over moving those repositories to the privileged pool, which would hand a permanent bypass to code that only ever needs one tenant.
- **The event-stream workers are per-tenant, not privileged.** `AnalyticsWorkerService` and `RecommendationWorkerService` each process one event for one user, so `handleEvent` wraps its whole body in `runAsTenant(event.userId, …)` on the app pool. `processed_events` is therefore granted (`RlsProcessedEventsGrant`) — it has no `user_id`, so it takes no policy, and the dedupe INSERT has to stay in the same transaction as the work it guards. Only `OutboxPublisher` still spans tenants among the stream consumers. **9 privileged consumers**, not 11.
- **Why the analytics worker checks habit ownership**: `habit_analytics` is keyed on `habit_id` alone. Before narrowing, an event naming a habit the user did not own fell through to defaults (`'UTC'`, `'daily'`, no logs) and wrote a row under the wrong tenant, which then blocked the real owner's upsert against the policy. `recomputeHabitAnalytics` now returns early when the habit lookup finds nothing.
- **Testing**: `rls.e2e-spec.ts` is the proof — unscoped selects, cross-tenant writes, missing context, partition coverage, worker tenant confinement, and a 60-query pooled-connection leak test. `cross-tenant.e2e-spec.ts` passes either way by design (it tests the user-visible contract), so it can't tell a working policy from an inert one; that's why both exist. Verified by disabling RLS on one table: 6 of the 15 fail. **Assert the mechanism, not just the outcome** — the worker-confinement tests pass on the application's own `user_id` filters even with the policy disabled, so each is paired with a direct forged write that only the database can refuse.
...
- **`psql` inside the container proves nothing about the password.** `pg_hba.conf` gives `127.0.0.1` **`trust`**, so a container-local `psql` connects whatever the password is; the app connects from the host and hits the `scram-sha-256` rule, which actually verifies. If the whole e2e suite fails with `password authentication failed for user "habitlab_app"`, the role and `.env` have diverged — test from the host (or via the app) to see it. `RlsAppRole` is already recorded in `migrations`, so `migrate` will not re-run and reset it; apply its `ALTER ROLE` branch by hand, per the runbook's rotation section.
- **The local test database is never truncated.** Nine e2e suites share it at `maxWorkers: 1` and every run adds users, habits and subscriptions, so time-sensitive suites (notably the notification scheduler, which scans every eligible habit) fail intermittently once volume builds. That is environmental, not a code regression — confirm against a clean checkout before chasing it.
